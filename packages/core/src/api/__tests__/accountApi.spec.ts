import { Http, HttpMockBuilder } from '@fruitsjs/http';
import {
...
import { generateFruitture, generateSignedTransactionBytes, verifyFruitture } from '@fruitsjs/crypto';
import { createChainService } from '../../__tests__/helpers/createChainService';
import { Amount, convertNumberToNQTString } from '@fruitsjs/util';
...
describe('AccountApi', () => {
    ...
    describe('generateSendTransactionQRCode', () => {
        const mockAddress = `FRUITS-K8MA-U2JT-R6DJ-FVQLC`;

        it('should expose a method for constructing the URL', async () => {
            // tslint:disable-next-line
            const expected = 'relPath?requestType=generateSendTransactionQRCode&receiverId=FRUITS-K8MA-U2JT-R6DJ-FVQLC&amountNQT=0&feeSuggestionType=standard';
    ...
    describe('getAliases', () => {
        ...
        it('should getAliases', async () => {
            const mockAlias: Alias = {
                account: '12345',
                accountRS: 'FRUITS-K8MA-U2JT-R6DJ-FVQLC`',
                alias: '12345',
                aliasName: 'test',
                aliasURI: 'acct:fruits-K8MA-U2JT-R6DJ-FVQLC@fruits',
                timestamp: 131932255
            };
    ...
    describe('setAccountInfo', () => {
        ...
        beforeEach(() => {
            ...
            // @ts-ignore
            generateFruitture = jest.fn(() => 'signature');
            // @ts-ignore
            verifyFruitture = jest.fn(() => true);
            // @ts-ignore
        ...
        it('should setAccountInfo', async () => {
            const status = await setAccountInfo(service)({
                ...
                feePlanck: Amount.fromFruit(1).getPlanck(),
                senderPublicKey: 'senderPublicKey',
                senderPrivateKey: 'senderPrivateKey'
            });
            expect(status).toBe('fakeTransaction');
            expect(generateFruitture).toBeCalledTimes(1);
            expect(verifyFruitture).toBeCalledTimes(1);
    ...
    describe('setRewardRecipient', () => {
        ...
        it('should throw error if response contains one', async () => {
            ...
            try {
                ...
                expect(generateFruitture).toBeCalledTimes(0);
                expect(verifyFruitture).toBeCalledTimes(0);
    ...
    describe('getAccountBlocks()', () => {
        it('should getAccountBlocks', async () => {
            httpMock = HttpMockBuilder.create().onGetReply(
                ...
                '/fruits?requestType=getAccountBlocks&account=accountId&firstIndex=1000&lastIndex=0&includeTransactions=true'
            ).build();
            const service = createChainService(httpMock);
        ...
        it('should getAccountBlockIds', async () => {
            httpMock = HttpMockBuilder.create().onGetReply(
                200,
                { blockIds: ['123', '456'] },
                '/fruits?requestType=getAccountBlockIds&account=accountId&firstIndex=1000&lastIndex=0&includeTransactions=true'
            ).build();
            const service = createChainService(httpMock);
            ...
            'subscriptions': [
                {
                    ...
                    'senderRS': 'FRUITS-K37B-9V85-FB95-793HN',
                    'recipient': '11089308770178933578',
                    'recipientRS': 'FRUITS-9AUC-LCKL-7W2D-B5BTM',
                    'amountNQT': '100000000',
                    'frequency': 3600,
                    'timeNext': 175560749
                }
    ...
    describe('add/removeCommitment()', () => {
        ...
        it('should add commitment', async () => {
            const amount = Amount.fromFruit(100);
            const fee = Amount.fromFruit(1);
