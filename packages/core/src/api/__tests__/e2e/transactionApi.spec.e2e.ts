...
import { HttpError } from '@fruitsjs/http';
import { FruitsValue, convertNumberToNQTString, FeeQuantPlanck } from '@fruitsjs/util';
...
describe('[E2E] Transaction Api', () => {
    ...
    it('should getTransaction', async () => {
        ...
        expect(transaction.senderRS.startsWith('FRUITS-')).toBeTruthy();
        ...
        expect(transaction.recipientRS.startsWith('FRUITS-')).toBeTruthy();
    ...
    it('should sendAmountToSingleRecipient', async () => {
        const transaction = await sendAmountToSingleRecipient(service)({
            ...
            amountPlanck: FruitsValue.fromFruits(1).getPlanck(),
            recipientId,
